...
<div style="max-width: 880px; display: flex; flex-direction: column; gap: 24px;">
    <div class="card" style="padding: 24px;">
        ...
        <div style="margin-top: 16px; padding: 12px; border: 1px solid var(--lilac-d); border-radius: var(--r-sm); background: var(--lilac); display: flex; align-items: center; gap: 8px; font-size: 12px; color: var(--ink-2);">
    ...
    {#if isOwner}
        <div class="card" style="padding: 24px;">
            ...
            {#if form?.ok && form?.inviteUrl}
                <div style="margin-top: 20px; padding: 16px; background: var(--surface-2); border-radius: var(--r-md); border: 1px dashed var(--hairline);">
                    <p style="font-size: 13px; margin-bottom: 8px;">Invite link created</p>
                    <div style="display: flex; gap: 8px; align-items: center;">
                        <code style="font-family: var(--font-mono); font-size: 12px; padding: 6px; background: var(--surface); border: 1px solid var(--hairline); flex: 1;">{form.inviteUrl}</code>
                        <Button onclick={() => copyToClipboard(form.inviteUrl)} size="sm" variant="outline"><Copy style="width: 14px; height: 14px;" /></Button>
                    </div>
                </div>
            {/if}
        </div>

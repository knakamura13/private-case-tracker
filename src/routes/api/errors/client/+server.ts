...
export const POST: RequestHandler = async (event) => {
    ...
    const url = typeof payload.url === 'string' ? redactUrl(clip(payload.url, MAX_URL_CHARS)) : null;
    const ua = typeof payload.userAgent === 'string' ? clip(payload.userAgent, MAX_UA_CHARS) : event.request.headers.get('user-agent');

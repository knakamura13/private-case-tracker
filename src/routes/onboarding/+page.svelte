...
<AuthShell
	...
	subtitle={data.isFirstUser
		? (step === 1 ? 'This will be the name of your shared workspace.' : step === 2 ? 'We will pre-load common tasks based on your selection.' : 'Just a few more things before we launch.')
	...
	{#if data.isFirstUser}
        ...
        <div style="display: flex; gap: 8px; margin-bottom: 32px;">
            {#each Array(totalSteps) as _, i}
                <div 
                    style="height: 4px; flex: 1; border-radius: 2px; transition: all 300ms ease; background: {i + 1 <= step ? 'var(--ink)' : 'var(--surface-3)'};"
                ></div>
            {/each}
        </div>
		...
		<form method="post" action="?/create" use:enhance>
            {#if step === 1}
                <div style="display: flex; flex-direction: column; gap: 16px;">
                    <div>
                        <label for="name" style="display: block; font-size: 13px; font-weight: 500; margin-bottom: 8px;">Workspace name</label>
                        <input 
                            id="name" 
                            name="name" 
                ...
                <div style="display: flex; flex-direction: column; gap: 12px; margin-bottom: 24px;">
                    {#each caseTypes as type}
                        <button 
                            ...
                            style="text-align: left; padding: 16px; border: 1px solid {caseType === type.id ? 'var(--peri-d)' : 'var(--hairline)'}; background: {caseType === type.id ? 'var(--peri)' : 'var(--surface)'}; border-radius: var(--r-md); transition: all 120ms ease; display: flex; align-items: center; justify-content: space-between;"
                        >
                            <div>
                            ...
                            {#if caseType === type.id}
                                <div style="width: 20px; height: 20px; border-radius: 50%; background: var(--peri-d); color: white; display: flex; align-items: center; justify-content: center;">
                ...
                <div style="display: flex; flex-direction: column; gap: 20px;">
                    <div class="card tinted-sage" style="padding: 16px; border-radius: var(--r-md);">
                        <p style="font-size: 13px; margin: 0; color: var(--sage-d);">
                            We've pre-configured your case for a <strong>{caseTypes.find(t => t.id === caseType)?.label}</strong>. 

...
function AddEditModal({ kind = 'task', entry, onClose }) {
    ...
    const config = {
        task: {
            ...
            labels: [
                { l: 'I-129F', c: 's-waiting' },
                { l: 'translation', c: 's-note' }
            ],
            due: 'Apr 25 · 1:52 PM',
            dueOverdue: true
        },
        ...
        question: {
            ...
            showThread: true,
            labels: [
                { l: 'lawyer', c: 's-waiting' },
                { l: 'passport', c: 's-active' }
            ],
            due: 'Asked Apr 22'
        }
    }[kind];
        ...
        <div className="modal-backdrop" onClick={onClose}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
                ...
                >
                    ...
                    <div style={{ flex: 1 }} />
                    <button className="btn ghost sm" style={{ width: 32, padding: 0, justifyContent: 'center' }}>
                        <IconImage size={16} />
                    </button>
                    <button className="btn ghost sm" style={{ width: 32, padding: 0, justifyContent: 'center' }}>
                        <IconEye size={16} />
                    ...
                    <button className="btn ghost sm" style={{ width: 32, padding: 0, justifyContent: 'center' }} onClick={onClose}>
                        <IconClose size={16} />
                    </button>
                </div>
                ...
                <div style={{ padding: 28, maxHeight: 720, overflowY: 'auto' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 24 }}>
                        ...
                        <button className="btn sm">
                            <IconTag size={13} /> Labels
                        </button>
                        {config.showSubtasks && (
                            <button className="btn sm">
                                <IconCheckSq size={13} /> Sub-tasks
                            </button>
                        )}
                        <button className="btn sm">
                            <IconClip size={13} /> Attachment
                        </button>
                        <button className="btn sm">
                            <IconLink size={13} /> Link
                        </button>
                        <button className="btn sm">
                            <IconDoc size={13} /> Evidence
                        </button>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28, marginBottom: 24 }}>
                        ...
                        <div>
                            ...
                            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                                <button className="btn sm">
                                    <IconCalendar size={13} /> {config.due}
                                </button>
                                {config.dueOverdue && (
                                    <span className="pill s-urgent" style={{ height: 26 }}>
                                        Overdue
                                    </span>
                                )}
                            </div>
                    ...
                    <div style={{ marginBottom: 24 }}>
                        ...
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                            {config.labels.map((l) => (
                                <span key={l.l} className={`pill ${l.c}`}>
                                    {l.l}
                                </span>
                            ))}
                            <button className="pill" style={{ borderStyle: 'dashed', background: 'transparent', cursor: 'pointer' }}>
                                + add
                            </button>
                        </div>
                    </div>
                    ...
                    <div style={{ marginBottom: 24 }}>
                        ...
                        >
                            {kind === 'task' &&
                                "Sworn translation needed for I-129F filing. Per USCIS, the translator must certify that they're competent to translate from Portuguese (BR) to English and that the translation is accurate."}
                            {kind === 'milestone' &&
                                'USPS Priority Mail Express, tracking 9405 5036 9930 0098 7714 22. Petition package included Form I-129F, $675 fee, two passport photos, and the bona fide relationship evidence binder (47 pages).'}
                            {kind === 'question' &&
                                "Sally's current Brazilian passport expires Feb 2027. The K-1 visa, if approved, would be issued for 6 months — does it need to remain valid 6 months past entry? Worth renewing now or waiting?"}
                        </div>
                    </div>
                    ...
                    {config.showSubtasks && (
                        <div style={{ marginBottom: 24 }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                                ...
                                <button className="btn sm" style={{ alignSelf: 'flex-start', marginTop: 6 }}>
                                    + Add a sub-task
                                </button>
                            </div>
                        </div>
                            ...
                            {[
                                {
                                    ...
                                    txt:
                                        kind === 'question'
                                            ? "Drafted this after checking the DS-160 instructions — let's bring it up Tuesday."
                                            : 'Found a translator on the USCIS approved list, $90/page.'
                                },
                                {
                                    who: 'a',
                                    txt:
                                        kind === 'question'
                                            ? "Mine expires Feb '27 — there's also a renewal queue of 3 weeks at the consulate right now."
                                            : 'Mailed the original this morning, photo is in evidence folder.'
                                }
                            ].map((c, i) => (
                                <div key={i} style={{ display: 'flex', gap: 12 }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                        <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 2 }}>
                                            {c.who === 'k' ? 'Kyle' : 'Sally'}{' '}
                                            <span className="mono" style={{ fontWeight: 400, color: 'var(--ink-3)', marginLeft: 6 }}>
                                                2d ago
                                            </span>
                                        </div>
                ...
                >
                    <button className="btn ghost sm" style={{ color: 'var(--blush-d)' }}>
                        Delete
                    </button>
                    <div style={{ flex: 1 }} />

...
const { Sidebar, Topbar, PageHeader, ByAvatar, StatusPill,
  IconDoc, IconQuestion, IconClip, IconPlus, IconChev, IconClock,
  IconMore, IconCheck, IconFilter, IconImage, IconFolder, IconLink,
  IconHeart2, IconShield, IconEye, IconCalendar } = window;
...
function EvidenceView() {
  const categories = [
    { id: "bona", title: "Bona fide relationship", count: 14, total: 18, tint: "sage", icon: IconHeart2,
      items: [
    ...
    { id: "forms", title: "Forms & supporting", count: 8, total: 12, tint: "butter", icon: IconShield,
      items: [
    ...
    { id: "vital", title: "Vital records", count: 2, total: 8, tint: "blush", icon: IconDoc,
      items: [
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "32px 32px 48px", flex: 1 }}>
          ...
          >
            ...
            <button className="btn primary"><IconPlus size={15} /> Add evidence</button>
          </PageHeader>
          ...
          <div className="bento" style={{ marginBottom: 12 }}>
            {categories.map(c => {
              const I = c.icon;
              return (
                <div key={c.id} className={`cat-card card tinted-${c.tint} span-4`} style={{ padding: 24, position: "relative" }}>
                  {/* Hover affordances */}
                  <div className="cat-actions" style={{
                    position: "absolute", top: 12, right: 12,
                    display: "flex", gap: 4, opacity: 0, transition: "opacity .15s",
                  }}>
                    <button className="icon-btn" title="Rename" style={{
                      ...
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 1 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>
                    </button>
                    <button className="icon-btn" title="Change color" style={{
                      width: 28, height: 28, borderRadius: 8, border: "1px solid var(--hairline)",
                      background: "var(--surface)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer",
                    }}>
                    ...
                    <button className="icon-btn" title="Delete category" style={{
                      width: 28, height: 28, borderRadius: 8, border: "1px solid var(--hairline)",
                      background: "var(--surface)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "var(--ink-2)",
                    }}>
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2v2"/></svg>
                    </button>
                  </div>
                  ...
                  <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 14 }}>
                    <span className="display" style={{ fontSize: 44, lineHeight: 1 }}>{c.count}</span>
                    <span className="mono" style={{ fontSize: 12, color: "var(--ink-2)" }}>/ {c.total} items</span>
                  </div>
                  <div style={{ height: 6, borderRadius: 4, background: "var(--surface)" }}>
                    <div style={{ width: `${c.count/c.total*100}%`, height: "100%", borderRadius: 4, background: "var(--ink)" }} />
                  </div>
                </div>
              );
            })}
          ...
          {categories.map(c => (
            <div key={c.id} className="cat-row card" style={{ padding: 0, marginBottom: 18, overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "16px 24px", borderBottom: "1px solid var(--hairline)" }}>
                <span className="dot" style={{ background: `var(--${c.tint}-fill)` }} />
                {/* contentEditable title — visible affordance via hover */}
                <div className="cat-title" style={{ fontSize: 14, fontWeight: 600, padding: "2px 6px", margin: "-2px -6px", borderRadius: 6, cursor: "text" }} suppressContentEditableWarning>{c.title}</div>
                <span className="mono" style={{ fontSize: 11, color: "var(--ink-3)" }}>{c.count} / {c.total}</span>
                <div style={{ flex: 1 }} />
                <div className="cat-row-actions" style={{ display: "flex", gap: 6, opacity: 0.3, transition: "opacity .15s" }}>
                ...
                <button className="btn sm"><IconPlus size={13} /> Add item</button>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, padding: 16 }}>
                {c.items.map((item, i) => (
                  <div key={i} style={{ display: "flex", gap: 12, padding: 14, borderRadius: 14, background: "var(--surface-2)" }}>
                    <div style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 4, lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.t}</div>
...
function QuestionsView() {
  const threads = [
    { id: "lawyer", title: "For the lawyer", color: "s-waiting", count: 4, sub: "Sandra Liu · next call May 5",
      items: [
        { q: "Does Sally need a new passport before consular interview?", who: "k", date: "Apr 22", replies: 2, tags: ["passport"] },
    ...
    { id: "interview", title: "Interview prep", color: "s-active", count: 6, sub: "São Paulo consulate · est. Q4 2026",
      items: [
    ...
    { id: "us", title: "Between us", color: "s-note", count: 2, sub: "Just for the two of us",
      items: [
    ...
    <div className="bloom" style={{ display: "flex", minHeight: 1080, background: "var(--bg)" }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "32px 32px 48px", flex: 1 }}>
          ...
          >
            <button className="btn primary"><IconPlus size={15} /> Add question</button>
          </PageHeader>
              ...
              <div key={thread.id} className="card" style={{ padding: 0, overflow: "hidden" }}>
                <div style={{ padding: 24, borderBottom: "1px solid var(--hairline)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                    ...
                    <div style={{ flex: 1 }} />
                    <span className="mono" style={{ fontSize: 11, color: "var(--ink-3)" }}>{thread.count}</span>
                  </div>
                ...
                <div style={{ padding: 12, display: "flex", flexDirection: "column", gap: 8 }}>
                  {thread.items.map((q, i) => (
                    <div key={i} style={{ padding: 14, borderRadius: 12, background: "var(--surface-2)" }}>
                      <div style={{ fontSize: 13, lineHeight: 1.45, marginBottom: 10, textWrap: "pretty" }}>{q.q}</div>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                        {q.tags.map(t => <span key={t} className="pill" style={{ height: 20, fontSize: 10, background: "var(--surface)" }}>{t}</span>)}
                        <div style={{ flex: 1 }} />
                        <ByAvatar who={q.who} />
                        <span className="mono" style={{ fontSize: 10, color: "var(--ink-3)" }}>{q.date}</span>
                        {q.replies > 0 && <span className="mono" style={{ fontSize: 10, color: "var(--ink-3)" }}>· {q.replies} replies</span>}
                      </div>
                    </div>
                  ))}

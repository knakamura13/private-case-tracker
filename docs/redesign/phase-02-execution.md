...
1.  **Update Navigation Items:** Ensure the `navigation` array matches this exactly:
    ...
    ```
    _Note: You may need to import `Link` from `lucide-svelte`._
...
<aside
    class="sidebar"
    style="width: 240px; padding: 28px 20px; background: var(--bg); border-right: 1px solid var(--hairline); display: flex; flexDirection: column; gap: 28px; flex-shrink: 0;"
>
    <!-- Header -->
            ...
            {#each navigation as item (item.href)}
                ...
                >
                    ...
                    {#if getPageNumber(item.href)}
                        <span
                            class="mono"
                            style="font-size: 11px; background: {active
                                ? 'oklch(0.30 0.02 270)'
                                : 'var(--surface-3)'}; padding: 2px 7px; border-radius: 999px; color: {active
                                ? 'var(--butter-fill)'
                                : 'var(--ink-2)'};"
                        >
    ...
    <div>
        ...
        <div style="display: flex; flex-direction: column; gap: 2px;">
            <a
                href="/settings"
                class="nav-item"
                style="display: flex; align-items: center; gap: 12px; padding: 10px 16px; border-radius: 12px; font-size: 14px; font-weight: 500; text-decoration: none; color: var(--ink-2);"
            >
                <Settings size={18} />
                <span>Settings</span>
            </a>
    ...
    <div style="margin-top: auto;">
        ...
        >
            ...
            <div style="font-size: 13px; font-weight: 600; margin-bottom: 4px; line-height: 1.3;">USCIS biometrics</div>
